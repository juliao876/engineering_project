...
const Rating: React.FC<RatingProps> = ({ projectId, initialAverage = 0, onAverageChange }) => {
  ...
  return (
    <div className="rating">
      ...
      <div className="rating__info">
        <span className="rating__average">{averageRating.toFixed(1)}</span>
        <span className="rating__label">
          {hoverRating ? `You are rating ${hoverRating}/5` : `Your rating: ${userRating || "–"}`}
        </span>
      </div>
      {status && <p className="rating__status">{status}</p>}
    </div>
  );
};
